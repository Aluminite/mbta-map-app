...
const MbtaMap = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            const result = await axios(
                'https://api-v3.mbta.com/routes?filter%5Btype%5D=0%2C1%2C2%2C3',
            );
            return result.data;
        }
    ...
    function updateSelectedRoute({currentTarget: dropdown}) {
        setSelectedRoute(dropdown.value);
        async function fetchData() {
            const result = await axios(
                'https://api-v3.mbta.com/vehicles?filter%5Broute%5D=' + dropdown.value,
            );
            return result.data;
        }
    ...
    return (
        <div>
            <Form.Select onChange={updateSelectedRoute}>
            ...
            <div className="map">
                <MapContainer center={[42.359149, -71.0581643]} zoom={10} scrollWheelZoom={true}>
                    ...
                    {routeVehicles.map(vehicle => (
                        <CircleMarker center={[vehicle.attributes.latitude, vehicle.attributes.longitude]} radius={10}></CircleMarker>
                    ))}
                </MapContainer>
            </div>
        </div>
